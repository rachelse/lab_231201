...
Slightly modified printing format of pdockq and ppv.
'''
...
#################MAIN####################
def pdockq(pdbfile):
    chain_coords, chain_plddt = read_pdb(pdbfile)
    #Check chains
    if len(chain_coords.keys())<2:
        print('Only one chain in pdbfile', pdbfile)
        sys.exit()
    ...
    return np.round(pdockq,3)

def convert_to_possibility(pae_points):
    def f(x):
        if x <= 10:
            return math.log10(-0.9*x+10)
        else:
            return -math.log10(0.45*x-3.5)
    possiblity = []
    for i in range(len(pae_points)):
        possiblity.append(f(pae_points[i]))
    return possiblity

def iPAE(interface_pair, paematrix, lenA):
    if len(interface_pair) == 0:
        return None
    else:
        pae_points = []
        for pair in interface_pair:
            a,b = pair[0]-1, lenA + pair[1]-1
            pae_points.append(paematrix[a][b])
            pae_points.append(paematrix[b][a])
        ipae = sum(convert_to_possibility(pae_points))
    return round(ipae,3)

def get_interface(chainA, chainB, threshold=8):
    ...
    for residueA in chainA:
        for residueB in chainB:
            if residueA['CA'] - residueB['CA'] <= threshold:
                # Save residue number of interface residues
                interface_pairs.append([residueA.get_id()[1], residueB.get_id()[1]])
                interfaceA.append(residueA.get_id()[1])
                interfaceB.append(residueB.get_id()[1])

    interfaceA = set(sorted(interfaceA))
    interfaceB = set(sorted(interfaceB))
    return interface_pairs, interfaceA, interfaceB

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Calculate interface PAE')
    parser.add_argument('-i', '--inputPDB', help='Input PDB file', type=str)
    parser.add_argument('-p', '--inputPAE', help='PAE file', type=str)
    parser.add_argument('-o', '--output', help = 'Output file', type = str)
    args = parser.parse_args()
    pdb_parser = PDBParser()

    with open(args.inputPAE, 'r') as f:
        paematrix = json.load(f)['pae']

    # TODO: Move interface extraction to other module
    name = args.inputPDB.split('/')[-1].split('_')[1]
    rank = args.inputPDB.split('/')[-1].split('_')[4]

    structure = pdb_parser.get_structure(name, args.inputPDB)
    model = structure[0]
    chainA = model['A']
    chainB = model['B']
    
    interface_pairs, interfaceA, interfaceB = get_interface(chainA, chainB)
    interface_score = iPAE(interface_pairs, paematrix, len(chainA))
    print(f'{name},{rank},{interface_score}')
